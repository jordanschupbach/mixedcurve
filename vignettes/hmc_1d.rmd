...
```{r, fig.filename="hmc_1d_data.png", fig.width=10, fig.height=10, out.width="98%", message=FALSE, warning=FALSE}
...
```{r, fig.filename="hmc_1d_fit.png", fig.width=20, fig.height=10, out.width="98%", message=FALSE, warning=FALSE}
...
parallel::clusterExport(cl, varlist = c("df1", "qseq", "bandwidth"))
...
cl <- parallel::makeCluster(parallel::detectCores() - 1)
parallel::clusterEvalQ(cl, { library(mixedcurve) })
parallel::clusterExport(cl, varlist = c("df1", "qseq", "bandwidth"))
wy_pvals <- mixedcurve::wy_full(dataf = df1,
                                ...
                                gen_pvals_fun = function(data, xseq) {
                                  lpk_res <- mixedcurve::lpkme(
                                    ...
                                    alternative_hypothesis = y ~ K_h(x1 | -1) + (K_h(x1) | grp2 / grp1),
                                    queries = xseq,
                                    data = data,
                                    degree = 0,
