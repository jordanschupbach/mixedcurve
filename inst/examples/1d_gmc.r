...
bs_true <- as.numeric(t(as.matrix(cbind(
  rnorm(nind, 0, 0.81),
  rnorm(nind, 0, 0.08),
  rnorm(nind, 0, 0.03)
))))
...
png("gmc_1d_poisson_data.png", width = 800, height = 800, res = 100)
mixedcurve::dark_mode()
plot(x, y,
  col = adjustcolor(id, 0.2), pch = 20,
...
glpk1 <- mixedcurve::glpkme(y ~ K_h(x) + (K_h(x) | grp2 / grp1),
  queries = qseq,
  data = df1,
  degree = 0,
...
qrs <- do.call(rbind, lapply(glpk1[[1]], function(elmt) {
  as.numeric(unlist(elmt$coefs))
}))
# 5. Plot the results
# png("vignettes/gmc_1d/gmc_1d_poisson_fit.png", width = 4800, height = 4800, res = 600)
#' png("gmc_1d_poisson_fit.png", width = 800, height = 800, res = 100)
mixedcurve::dark_mode()
plot(df1$x, df1$y,
