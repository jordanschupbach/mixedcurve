library(mixedcurve)
library(ParBayesianOptimization)
set.seed(123)
nt <- c(100)
ni <- c(10)
df1 <- mixedcurve::gen_hfanova_data(
  f = function(t, i) {
    3.2 * mixedcurve::m3(t, i)
  },
  n = c(nt, ni), sigmas = c(0.01, 0.15), bounds = c(0, 1),
  ngrp = 3,
  ndim = 1, px = NULL, pxargs = NULL,
  family = "gaussian", white_noise = TRUE
)
par(mfrow = c(3, 1))
mixedcurve::dark_mode()
plot(df1$x1, df1$y,
  main = "Cuevas M2 function data (1)",
  ylab = "y", xlab = "x", pch = 20, col = adjustcolor(df1$cov, 0.1)
)
subsetdf <- function(dataf, dnames, query, h) {
  if (length(dnames) != length(query)) {
    stop("Length of dnames and query must be the same")
  }
  if (!all(dnames %in% names(dataf))) {
    stop("Some dnames are not found in the dataframe")
  }
  queries <- t(replicate(nrow(dataf), as.numeric(query), simplify = "matrix"))
  distances <- NULL
  if (length(query) > 1) {
    distances <- sqrt(apply(
      matrix(unlist(lapply(seq_len(nrow(dataf)), function(i) {
        dataf[i, dnames] - queries[i, ]
      })), nrow = nrow(dataf), byrow = TRUE)^2,
      1, sum
    ))
  } else {
    distances <- abs(dataf[, dnames] - as.numeric(query))
  }
  dataf[distances <= h, , drop = FALSE]
}

nq <- 101
qseq <- seq(0, 1, length.out = nq)
bandwidth <- 0.04
tstats <- numeric(nrow(as.matrix(qseq)))
qseq_fit <- numeric(nrow(as.matrix(qseq)))
pvals <- numeric(nrow(as.matrix(qseq)))
cl <- parallel::makeCluster(parallel::detectCores() - 1)
parallel::clusterExport(cl, varlist = c("df1", "subsetdf", "qseq", "bandwidth"))
qs <- parallel::parLapply(seq_len(nrow(as.matrix(qseq))), function(i) {
  df_sub <- subsetdf(df1, c("x1"), qseq[i], h = bandwidth)
  model1 <- lme4::lmer(y ~ cov + (1 | grp1), data = df_sub)
  model0 <- lme4::lmer(y ~ -1 + (1 | grp1), data = df_sub)
  qseq_fit <- lme4::fixef(model1)
  pval <- anova(model0, model1)$`Pr`[2]
  tstat <- sqrt(anova(model0, model1)$Chisq[2])
  mse <- mean(residuals(model1)^2)
  list(fit = qseq_fit, mse = mse, pval = pval, tstat = tstat)
}, cl = cl)
parallel::stopCluster(cl)
fits <- matrix(unlist(lapply(qs, function(elmt) elmt$fit)),
  nrow = nq, byrow = TRUE
)
pvals <- unlist(lapply(qs, function(elmt) elmt$pval))
tstats <- unlist(lapply(qs, function(elmt) elmt$tstat))
msevals <- unlist(lapply(qs, function(elmt) elmt$mse))
sum(msevals) / diff(qseq)[1]

mean_cv_error <- function(h) {
  k <- 5
  folds <- sample(1:k, nrow(df1), replace = TRUE)
  msecv <- numeric(k)
  for (i in 1:k){
    tdf <- df1[folds != i, ]
    tdf2 <- df1[folds == i, ]
    nq <- 101
    qseq <- seq(0, 1, length.out = nq)
    bandwidth <- h
    cl <- parallel::makeCluster(parallel::detectCores() - 1)
    parallel::clusterExport(cl, varlist = c("df1", "subsetdf",
                                            "qseq", "bandwidth"))
    qs <- parallel::parLapply(seq_len(nrow(as.matrix(qseq))), function(i) {
      df_sub <- subsetdf(tdf, c("x1"), qseq[i], h = bandwidth)
      df_sub2 <- subsetdf(tdf2, c("x1"), qseq[i], h = bandwidth)
      model1 <- lme4::lmer(y ~ cov + (1 | grp1), data = df_sub)
      preds <- predict(model1, newdata = df_sub2)
      mse <- mean((preds - df_sub2$y)^2)
      list(mse = mse)
    }, cl = cl)
    parallel::stopCluster(cl)
    msevals <- unlist(lapply(qs, function(elmt) elmt$mse))
    msecv[i] <- sum(msevals) / diff(qseq)[1]
  }
  - mean(msecv)
}

optfun <- function(x) {
  tryCatch(
    list(Score = mean_cv_error(x)),
    error = function(e) {
      list(Score = -999)
    }
  )
}
bounds <- list(x = c(0.02, 0.2))
init_grid <- data.frame(x = c(0.02, 0.05, 0.1))
set.seed(6)
bopt <- bayesOpt(
  FUN = optfun,
  bounds = bounds,
  initGrid = init_grid,
  iters.n = 15
)
bandwidth <- getBestPars(bopt)$x
bandwidth

cl <- parallel::makeCluster(parallel::detectCores() - 1)
parallel::clusterExport(cl, varlist = c("df1", "subsetdf", "qseq", "bandwidth"))
qs <- parallel::parLapply(seq_len(nrow(as.matrix(qseq))), function(i) {
  df_sub <- subsetdf(df1, c("x1"), qseq[i], h = bandwidth)
  model1 <- lme4::lmer(y ~ cov + (1 | grp1), data = df_sub)
  model0 <- lme4::lmer(y ~ -1 + (1 | grp1), data = df_sub)
  qseq_fit <- lme4::fixef(model1)
  pval <- anova(model0, model1)$`Pr`[2]
  tstat <- sqrt(anova(model0, model1)$Chisq[2])
  mse <- mean(residuals(model1)^2)
  list(fit = qseq_fit, mse = mse, pval = pval, tstat = tstat)
}, cl = cl)
parallel::stopCluster(cl)
fits <- matrix(unlist(lapply(qs, function(elmt) elmt$fit)),
  nrow = nq, byrow = TRUE
)
pvals <- unlist(lapply(qs, function(elmt) elmt$pval))
tstats <- unlist(lapply(qs, function(elmt) elmt$tstat))
par(mfrow = c(3, 1))
plot(df1$x1, df1$y,
  main = "Cuevas M2 function data (1)",
  ylab = "y", xlab = "x", pch = 20, col = adjustcolor(df1$cov, 0.1)
)
lines(qseq, fits[, 1])
lines(qseq, fits[, 1] + fits[, 2], col = 2)
lines(qseq, fits[, 1] + fits[, 3], col = 3)
plot(qseq, pvals, type = "l", ylim = c(0, 1))
nb <- 100
boot_tstats <- matrix(NA, nrow = nb - 1, ncol = nq)
boot_pvals <- matrix(NA, nrow = nb - 1, ncol = nq)
system.time({
  cl <- parallel::makeCluster(parallel::detectCores() - 1)
  parallel::clusterExport(cl, varlist = c(
    "df1", "subsetdf",
    "qseq", "bandwidth"
  ))
  for (i in seq_len(nrow(as.matrix(qseq)))) {
    df_sub <- subsetdf(df1,
      dnames = c("x1"),
      qseq[i], h = bandwidth
    )
    parallel::clusterExport(cl, varlist = c("df_sub"))
    bootstraps <- parallel::parLapply(1:(nb - 1), function(b) {
      df_sub$pcov <- sample(df_sub$cov, replace = TRUE)
      model1 <- lme4::lmer(y ~ pcov + (1 | grp1),
        data = df_sub
      )
      model0 <- lme4::lmer(y ~ 1 + (1 | grp1), data = df_sub)
      anova_res <- anova(model1, model0)
      stat <- anova_res$Chisq[2]
      pval <- anova_res$P[2]
      list("chisq" = stat, "pval" = pval)
    }, cl = cl)
    boot_tstats[, i] <- unlist(lapply(bootstraps, function(elmt) elmt$chisq))
    boot_pvals[, i] <- unlist(lapply(bootstraps, function(elmt) elmt$pval))
  }
  parallel::stopCluster(cl)
})
system.time({
  rw_adj_pvals <- mixedcurve::romano_wolf(tstats, boot_tstats)
  wy_adj_pvals <- mixedcurve::westfall_young(pvals, t(boot_pvals))
})
plot(qseq, pvals, type = "l", lty = 2, col = 1, ylim = c(0, 1))
lines(qseq, rw_adj_pvals, type = "l", lty = 2, col = 2)
lines(qseq, wy_adj_pvals, type = "l", lty = 1, col = 3)
lines(
  qseq, p.adjust(pvals,
    method = "BY",
    n = length(pvals)
  ),
  type = "l", col = 4
)
lines(
  qseq, p.adjust(pvals,
    method = "holm",
    n = length(pvals)
  ),
  type = "l", col = 5
)
lines(
  qseq, p.adjust(pvals,
    method = "hochberg",
    n = length(pvals)
  ),
  type = "l", col = 6
)
lines(
  qseq, p.adjust(pvals,
    method = "hommel",
    n = length(pvals)
  ),
  type = "l", col = 7
)
lines(
  qseq, p.adjust(pvals,
    method = "BH",
    n = length(pvals)
  ),
  type = "l", col = 8
)

